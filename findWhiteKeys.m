...
avg_blackXDiff = mean(blackXDiff);
peaks = zeros([1, length(blackXDiff)]);
peaks(blackXDiff > avg_blackXDiff) = 1;
csharp_indicies = findstr(peaks, [1,0,1,0,0]);
center_index = length(blackXDiff) / 2;
csharp_distance = abs(center_index - csharp_indicies);
[tmp,middle_csharp_pos] = min(csharp_distance)
middle_csharp = csharp_indicies(middle_csharp_pos) + 1;
